...
async def hugs(message: Message):
    ...
    if not username and not reply:
        await message.edit("**Bruh** ~`You shouldn't hug/kiss yourself ಠಗಠ`", del_in=3)
        return

    kwargs: dict = {"reply_to_message_id": reply_id, "unsave": unsave}

    if "-g" in message.flags:
        await message.client.send_animation(
            message.chat.id,
            await get_link("-g"),
            caption=f"**UwU hugged {username} !**",
            **kwargs,
        )
        ...
        await message.client.send_animation(
            message.chat.id,
            await get_link("-w"),
            caption=f"**UwU winked at {username} !**",
            **kwargs,
        )
        ...
        await message.client.send_animation(
            message.chat.id,
            await get_link("-s"),
            caption=f"**Slapped {username}**",
            **kwargs,
        )
    else:
        kwargs.pop("unsave")
        await message.reply_photo(
            await get_link("-d"), caption=f"**UwU hugged {username} !**", **kwargs
        )
