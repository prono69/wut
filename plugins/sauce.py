...
async def get_file_ext(filename):
    proc = await asyncio.create_subprocess_exec(
        "file", "--brief", "--extension", filename, stdout=asyncio.subprocess.PIPE
    )
...
async def get_file_mimetype(filename):
    ...
    if not mimetype:
        proc = await asyncio.create_subprocess_exec(
            "file", "--brief", "--mime-type", filename, stdout=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
...
async def saucenao(message: Message):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        filename = await message.client.download_media(
            media, file_name=os.path.join(tempdir, "0")
        )
        mimetype = await get_file_mimetype(filename)
        ...
        if mimetype.startswith("video/"):
            ...
            proc = await asyncio.create_subprocess_exec(
                "ffmpeg", "-an", "-sn", "-i", filename, new_path
            )
            await proc.communicate()
            filename = new_path
        with open(filename, "rb") as file:
            async with session.post(
                f"https://saucenao.com/search.php?db=999&output_type=2&api_key={urlencode(saucenao_api)}",
                data={"file": file},
            ) as resp:
                json = await resp.json()
        if json["header"]["status"]:
            await message.edit(
                f'<b>{json["header"]["status"]}:</b> {html.escape(json["header"].get("message", "No message"))}'
            )
            return
        ...
        for result in json["results"]:
            ...
            low_similarity = (
                Decimal(result["header"]["similarity"]) < minimum_similarity
            )
            if low_similarity:
                ...
                for url in result["data"]["ext_urls"]:
                    if result["header"]["index_id"] in (5, 6):
                        ...
                        if qs.get("illust_id"):
                            async with session.get(
                                f'https://www.pixiv.net/touch/ajax/illust/details?illust_id={urlencode(qs["illust_id"][0])}',
                            ...
                            if json["body"]:
                                ...
                                for i in (
                                    ...
                                    if (
                                        pimg
                                        and await download_file(pimg, filename, url)
                                        and os.path.getsize(filename) < 10000000
                                    ):
                    ...
                    if await download_file(url, filename):
                        ...
                        pimg = soup.find(
                            ...
                            and tag.attrs.get("property") == "og:image"
            ...
            atext += "\n\n"
            length = len(
                (await message.client.parser.parse(caption + atext, "html"))["message"]
            )
            if length <= 1024:
                caption += atext
            if length < 4096:
